...
const parsers = [
	...
	},
	{
		...
		parser: (body) => {
			...
			//
			// 카드번호와 상태 사이의 공백, 그리고 시각은 오기도 하고 안 오기도
			// 한다. 실측 원문 둘을 나란히 두면 차이가 보인다.
			//
			//   'KB국민카드6036승인 김*심님 14,160원 일시불 09/02 16:13 11번가 누적120,830'
			//   'KB국민카드8031 승인 김*심 25,790원 09/22 삼천리(주)'
			//
			// 둘 다 필수로 보던 동안 아래 형식이 no-transaction 으로 떨어졌다.
			// 시각을 선택으로 둬도 상호가 밀리지는 않는다 — 정규식이 있으면 먼저
			// 시각으로 붙인다.
			//
			// 누적 필드에 접두어가 붙는 경우까지 받는다 — NH 는 '총누적' 으로
			// 실측됐고, 안 떼면 상호에 그대로 붙어 카테고리 분류까지 틀어진다.
			const card = line.match(
				/KB국민카드([0-9*]+)\s*(승인|취소)\s+\S+\s+([\d,]+)원(?:\s+(?!\d{2}\/\d{2})\S+)*\s+(\d{2}\/\d{2})(?:\s+\d{1,2}:\d{2})?\s+(.+?)(?:\s+\S*누적[\d,]+원?)?\s*$/
			);
			if (card) {
		...
		parser: (body) => {
			...
			return {
				...
				transaction: {
					...
					// 상호 뒤에 그 달 누적 사용액이 붙어 온다. 여는 괄호가 붙기도
					// 하고('키다리식품 ( 누적41,290원') 안 붙기도 한다
					// ('화성시청 누적47,790원') — 괄호를 필수로 보면 후자에서
					// 누적액이 상호에 그대로 남는다.
					//
					// 상호 자체에 괄호가 들어가는 경우가 있어('（유）아웃백')
					// 괄호를 통째로 떼지 않고 누적 꼬리만 집어서 떼어 낸다.
					// KB·NH 처럼 '총누적' 같은 접두어가 붙어도 받는다.
					payee: m[3].replace(/\s*\(?\s*\S*누적\s*[\d,]+\s*원?[\s\S]*$/, '').trim(),
					category: '분류없음'
				}
			};
		}
